...
const getOpenAIKey = () => {
  ...
  // For development: Add EXPO_PUBLIC_OPENAI_API_KEY=your_key_here to your .env file
  const HARDCODED_DEV_KEY = 'your-openai-api-key-here'; // Replace with your actual API key
  
  // Debug: Log all environment variables that start with EXPO_PUBLIC
  console.log('🔍 Debug: All EXPO_PUBLIC env vars:', Object.keys(process.env).filter(key => key.startsWith('EXPO_PUBLIC')));
  
  // First try environment variable
  let apiKey = process.env.EXPO_PUBLIC_OPENAI_API_KEY;
  
  // Debug: Log the raw value
  console.log('🔍 Debug: Raw API key from env:', apiKey ? `"${apiKey}"` : 'undefined');
  console.log('🔍 Debug: API key length:', apiKey ? apiKey.length : 0);
  console.log('🔍 Debug: API key starts with sk-proj:', apiKey ? apiKey.startsWith('sk-proj') : false);
  
  // If no environment variable, use hardcoded key for development
  if (!apiKey || apiKey === 'your-openai-api-key-here') {
    console.log('🔧 Using hardcoded API key for development');
    apiKey = HARDCODED_DEV_KEY;
  }
  
  if (!apiKey || apiKey === 'your-openai-api-key-here') {
    console.warn('⚠️ OpenAI API key not found. Please set EXPO_PUBLIC_OPENAI_API_KEY in your .env file or replace HARDCODED_DEV_KEY');
    return null;
  }
